...
export default function KYCVerification() {
  ...
  const fetchToken = useCallback(async (): Promise<string> => {
    ...
    if (!resp.ok) {
      ...
      throw new Error((err as { detail?: string })?.detail ?? 'שגיאה בקבלת טוקן אימות')
    }
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        ...
        <p style={{ color: '#94a3b8', fontSize: 14, marginTop: 8 }}>
          {driverType === 'licensed_taxi' ? 'אימות לרישיון נהג מונית' : 'אימות לנהג שיתופי'}
        </p>
        <button
          onClick={() => navigate('/driver?tab=verify')}
          style={{
            marginTop: 12,
      ...
      {status === 'success' && (
        <div style={{
          ...
          <p style={{ color: '#94a3b8' }}>מעביר אותך לדאשבורד...</p>
        </div>
      )}
