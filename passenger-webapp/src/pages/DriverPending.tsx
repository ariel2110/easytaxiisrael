...
interface Step { label: string; desc: string; status: 'done' | 'active' | 'idle' }

type KycStatus = 'not_started' | 'created' | 'started' | 'completed' | 'approved' | 'declined' | 'expired'

function stepsFromKyc(kycStatus: KycStatus): Step[] {
  const isActive = (s: KycStatus) => ['created', 'started'].includes(s)
  const isDone   = (s: KycStatus) => ['completed', 'approved'].includes(s)
...
export default function DriverPending() {
  ...
  useEffect(() => {
    ...
    el.id = 'dp-css'; el.textContent = CSS
  ...
  return (
    <div className="dp">
      ...
      <div className="dp-wrap">
        ...
        {/* Spinner icon */}
        <div className="dp-icon-wrap dp-ain">
          <div className="dp-pulseRing" />
          <div className="dp-icon">🪪</div>
        </div>

        <div className="dp-card dp-ain dp-d1">
          {approved ? (
            <>
              <div className="dp-title">🎉 <span>אושרת!</span></div>
              <div className="dp-sub">הזהות אומתה בהצלחה. אתה מוכן לקבל נסיעות!</div>
              <button
                className="dp-wa"
                style={{ background: '#2563EB', boxShadow: '0 4px 18px rgba(37,99,235,.4)' }}
                onClick={() => { window.location.href = 'https://driver.easytaxiisrael.com' }}
              >
                🚕 פתח פורטל נהגים
              </button>
            </>
          ...
          <div className="dp-eta">⏱ זמן ממוצע לאישור: עד {etaHours} שעות</div>

          {/* Steps */}
          <div className="dp-steps">
            {steps.map(s => (
              <div className={`dp-srow ${s.status}`} key={s.label}>
                <div className="dp-sbadge">
                  {s.status === 'done' ? '✓' : s.status === 'active' ? '⚙' : '○'}
                </div>
                <div>
                  <div className="dp-slabel">{s.label}</div>
                  <div className="dp-sdesc">{s.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Contact + nav */}
        <div className="dp-ain dp-d2">
          <a
            href="https://wa.me/447474775344?text=%D7%A9%D7%9C%D7%95%D7%9D%2C%20%D7%90%D7%A0%D7%99%20%D7%A0%D7%94%D7%92%20%D7%9E%D7%97%D7%9B%D7%94%20%D7%9C%D7%90%D7%99%D7%A9%D7%95%D7%A8"
            target="_blank"
          ...
          <a className="dp-link" onClick={() => navigate('/')} style={{ cursor: 'pointer' }}>
            ← חזור לדף הבית
          </a>
        </div>
