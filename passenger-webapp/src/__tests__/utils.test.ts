...
describe('Surge multiplier display guard', () => {
  const shouldShowSurge = (multiplier: string | undefined | null): boolean => {
...
describe('Phone display normalization', () => {
  const normalizeForDisplay = (phone: string): string => {
...
describe('Fare calculation', () => {
  const calculateFare = (distKm: number, surgeMultiplier = 1) => {
    const BASE_FARE = 12
    const PER_KM = 3.5
