...
export function useAuth() {
    ...
    const requestWaAuth = useCallback(async (phone, role = 'passenger') => {
        setError(null);
        const res = await api.auth.requestWaAuth(phone, role);
        ...
        async function doPoll() {
            try {
                ...
                if (poll.status === 'completed' && poll.access_token && poll.refresh_token) {
                    ...
                    if (poll.role)
                        localStorage.setItem('auth_role', poll.role);
                    else
    ...
    const verifyOtp = useCallback(async (otp, role = 'passenger') => {
        if (!otpPhone)
            return;
        setError(null);
        ...
        if (res.role)
            ...
            localStorage.removeItem('auth_role');
        if (res.kyc_url)
            localStorage.setItem('kyc_url', res.kyc_url);
        else
            localStorage.removeItem('kyc_url');
        setOtpPhone(null);
        try {
