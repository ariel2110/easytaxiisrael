/**
 * RatingModal (driver-webapp) — driver rates the passenger after ride ends.
 */
...
export default function RatingModal({ rideId, onDone }: Props) {
      ...
      <div style={{
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
            ...
            <div style={{ fontWeight: 700, color: '#22c55e', fontSize: '1.1rem' }}>תודה על הדירוג!</div>
          </div>
        ) : (
          <>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              <div style={{ fontSize: '1.75rem', marginBottom: '.5rem' }}>🏁</div>
              <div style={{ fontWeight: 800, fontSize: '1.1rem', color: '#F1F5F9' }}>הנסיעה הסתיימה!</div>
              <div style={{ color: '#94A3B8', fontSize: '.875rem', marginTop: '.25rem' }}>כיצד היה הנוסע?</div>
            </div>
            ...
            {(hover || score) > 0 && (
              <div style={{ textAlign: 'center', fontSize: '.8rem', color: '#F59E0B', marginBottom: '1rem', fontWeight: 600 }}>
                {['', 'גרוע', 'לא טוב', 'בסדר', 'טוב מאוד', 'מעולה!'][hover || score]}
              </div>
            )}
